...
template <typename T>
Vector3<T> MultibodyTree<T>::CalcCenterOfMassVelocity(
    const systems::Context<T>& context) const {
  if (!(num_bodies() > 1)) {
    throw std::runtime_error(
  ...
  return CalcCenterOfMassVelocity(context, model_instances);
}
...
Vector3<T> MultibodyTree<T>::CalcCenterOfMassVelocity(
  ...
  return CalcCenterOfMassVelocity(context, body_indexes);
}
...
Vector3<T> MultibodyTree<T>::CalcCenterOfMassVelocity(
  ...
  for (BodyIndex body_index : body_indexes) {
    ...
    Vector3<T> pi_WBcm = body.EvalPoseInWorld(context) * pi_BoBcm;
    Vector3<T> vi_WBcm =
        body.EvalSpatialVelocityInWorld(context).Shift(pi_WBcm).translational();

    // Calculate composite_mass and M * v in world frame.
    const T& body_mass = body.get_mass(context);
    // Mv = ∑ mᵢ * vi_WBcm
    Mv += body_mass * vi_WBcm;
    // composite_mass = ∑ mᵢ
    composite_mass += body_mass;
  }
  ...
  if (!(composite_mass > 0)) {
    throw std::runtime_error(
        "CalcCenterOfMassVelocity(): the total mass must larger than zero.");
  }
  return Mv / composite_mass;
}
